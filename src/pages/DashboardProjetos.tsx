...
const DashboardProjetos = () => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Dashboard Projetos');
    const fileName = selectedSprints.length === 1 ? `dashboard_projetos_${selectedSprintsData[0].nome.replace(/\s+/g, '_')}_${format(new Date(), 'yyyy-MM-dd')}.xlsx` : `dashboard_projetos_multiplas_sprints_${format(new Date(), 'yyyy-MM-dd')}.xlsx`;
