...
const DashboardClientes = () => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Dashboard Clientes');
    const fileName = selectedSprints.length === 1 ? `dashboard_clientes_${selectedSprintsData[0].nome.replace(/\s+/g, '_')}_${format(new Date(), 'yyyy-MM-dd')}.xlsx` : `dashboard_clientes_multiplas_sprints_${format(new Date(), 'yyyy-MM-dd')}.xlsx`;
